...
public class ExecutionManager {
   ...
   */
  @NonNull
  public TplRenderExecution startRenderExecution(
      String datasourceId,
      String templateId,
      String filename,
    ...
    try {
      ...
      renderExecution.setFilename(
          StringUtils.hasText(filename) ? filename : template.getDefFilename());
  ...
  public void listenRenderExecutionListener(RenderExecutionEvent event) {
    ...
    this.renderExecutionService
        .updateChain()
        .eq(FieldConstants.PRIMARY_KEY, renderExecution.getId())
        .set(TplRenderExecutionTableDef.TPL_RENDER_EXECUTION.STATUS, ExecutionStatus.PENDING)
        .update();
    // 创建结果
    ...
    try {
      ...
      List<? extends IAttachmentModel> atts =
          templateService.getAttachmentList(renderExecution.getTemplateId());
      if (atts.size() > 0) {
        ...
        AttachmentVo attachmentVo =
            this.renderAndUpload(
                ...
                renderExecution.getTemplateName(),
                renderExecution.getFilename(),
                context);
        // 保存结果
        ...
        if (ret) {
          ...
          this.renderExecutionService
              .updateChain()
              .eq(FieldConstants.PRIMARY_KEY, renderExecution.getId())
              .set(TplRenderExecutionTableDef.TPL_RENDER_EXECUTION.STATUS, ExecutionStatus.FAILED)
              .update();
        }
      } else {
  ...
  private void checkDatasourceParams(String datasourceId, @NonNull DatasourceParamHolder params) {
    List<TplDatasourceParam> datasourceParamList =
        this.datasourceService.getDatasourceParams(datasourceId);
    datasourceParamList.forEach(
        dp -> {
          // 检查必填
   ...
   */
  @NonNull
  private AttachmentVo renderAndUpload(
      @NonNull ITemplateRender templateRender,
      String templateAttachmentId,
      String templateName,
      String filename,
    ...
    try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
      ...
      FileInfo fileInfo =
          this.fileStorageService
              .of(outputStream.toByteArray(), filename, templateRender.getContentType())
              .setPath(this.getResultUploadPath())
              .upload();
  ...
  @NonNull
  private RenderContext buildContext(@NonNull TplRenderExecution execution, Object dataModel)
    ...
    DatasourceParamHolder params =
        this.objectMapper.readValue(execution.getParams(), DatasourceParamHolder.class);
    return new RenderContext(
        execution.getTemplateName(), execution.getDatasourceName(), params, envs, dataModel);
  }
}
