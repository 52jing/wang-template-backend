...
public class ReportAnalysisController {
  ...
  public ResponseEntity<?> indicatorAnalysis(MultipartFile file) {
    if (!analysisProperties.isIndicatorAnalysis()) {
      throw new ErrorCodeException(ErrorCode.ANALYSIS_NOT_ENABLED);
    }
    // 允许的上传格式
    String[] restrictContentTypes = new String[]{"application/pdf", "image/", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"};
    ...
    if (Arrays.stream(restrictContentTypes)
      ...
      try {
        ...
        return ResponseUtils.success(DetailBody.ok(analysisManager.analysisIndicators(bytes, filename, contentType)));
      } catch (IOException e) {
        throw new ErrorCodeException(HttpErrorCode.BAD_REQUEST);
      } catch (MoonShotRequestException e) {
        throw new ErrorCodeException(ErrorCode.ANALYSIS_FAILED);
      }
    }
