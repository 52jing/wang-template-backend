...
CREATE TABLE wb_sys_frontend(
    ...
    `allow_register` INT(1) NOT NULL  COMMENT '是否允许注册' ,
    `staff_only` INT(1) NOT NULL  COMMENT '是否内部使用' ,
    PRIMARY KEY (id)
)  COMMENT = '前端';
...
CREATE TABLE wb_sys_user(
    ...
    `superuser` INT(1) NOT NULL  COMMENT '是否超级管理员' ,
    `staff` INT(1) NOT NULL  COMMENT '是否内部用户' ,
    ...
    `need_change_pwd` INT(1) NOT NULL  COMMENT '是否需要修改密码' ,
    PRIMARY KEY (id)
)  COMMENT = '用户';
...
CREATE TABLE wb_sys_policy(
    ...
    `readonly` INT(1) NOT NULL  COMMENT '是否只读' ,
    PRIMARY KEY (id)
)  COMMENT = '策略';
...
CREATE TABLE wb_sys_announcement(
    ...
    `content` TEXT NOT NULL  COMMENT '内容' ,
    `type` VARCHAR(32) NOT NULL  COMMENT '类型' ,
    `display` INT(1) NOT NULL  COMMENT '是否展示' ,
    `sort` INT NOT NULL  COMMENT '排序' ,
    PRIMARY KEY (id)
)  COMMENT = '公告';
...
CREATE TABLE wb_sys_attachment(
    ...
    `th_size` bigint(20) NOT NULL  COMMENT '缩略图大小' ,
...
CREATE TABLE wb_sys_bg_task(
    ...
    `type` VARCHAR(32) NOT NULL  COMMENT '类型' ,
    `status` VARCHAR(32) NOT NULL  COMMENT '状态' ,
    `attachment_id` VARCHAR(64)   COMMENT '关联附件ID' ,
